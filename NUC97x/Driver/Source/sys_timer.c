...
void sysTimer0ISR()
{
    ...
    if (inpw(REG_TMR_TISR) & 0x00000001)
    {
        ...
        outpw(REG_TMR_TISR, 0x01); // clear TIF0 
			...
			//注:非32位满自动回0,而各以太网、USB等定时用32位二个值减会有问题.(实测回0后以太网就不通了,出现过但没重复出现)
...
void sysTimer1ISR()
{
    ...
    if (inpw(REG_TMR_TISR) & 0x00000002)
    {
        ...
        outpw(REG_TMR_TISR, 0x02); // clear TIF1 
			...
			//注:非32位满自动回0,而各以太网、USB等定时用32位二个值减会有问题
...
INT32 sysStartTimer(INT32 nTimeNo, UINT32 uTicksPerSecond, INT32 nOpMode)
{
    ...
    switch (nTimeNo)
    {
        case TIMER0://TIMER0同时用于eth0,eth1和usb0h和FMI_NAND
					
						if( _sys_bIsTimer0Initial == TRUE)
						{
							return 0;
						}
            ...
            _sys_pvOldTimer0Vect = sysInstallISR(HIGH_LEVEL_SENSITIVE | IRQ_LEVEL_7, TMR0_IRQn, (PVOID)sysTimer0ISR);
            sysEnableInterrupt(TMR0_IRQn);

					#if DEBUG==0//0=发行,1=调试
            _sys_uTimer0Count = 0;
					#else
						_sys_uTimer0Count=0xffffffff-(60000/10);//置初值用于sysGetTicks()回0测试
					#endif
        ...
        case TIMER1://暂没用,FreeRTOS另
            ...
            _sys_pvOldTimer1Vect = sysInstallISR(HIGH_LEVEL_SENSITIVE | IRQ_LEVEL_7, TMR1_IRQn, (PVOID)sysTimer1ISR);
...
INT32 sysStopTimer(INT32 nTimeNo)
{
    switch (nTimeNo)
    {
        case TIMER0:
            ...
            sysInstallISR(HIGH_LEVEL_SENSITIVE | IRQ_LEVEL_7, TMR0_IRQn, _sys_pvOldTimer0Vect);
        ...
        case TIMER1:
            ...
            sysInstallISR(HIGH_LEVEL_SENSITIVE| IRQ_LEVEL_7, TMR1_IRQn, _sys_pvOldTimer1Vect);
...
INT32 sysSetTimerEvent(INT32 nTimeNo, UINT32 uTimeTick, PVOID pvFun)
{
    ...
    switch (nTimeNo)
    {
        case TIMER0://eth0,eth1,USB0H,FMI_NAND
						//会重复设置,先检查原是否已经存在
						if( _sys_bIsSetTime0Event == TRUE)
						{
							for (i=0; i<TimerEventCount; i++)
							{
								if (tTime0Event[i].active == TRUE)
								{
									if(tTime0Event[i].funPtr ==(sys_pvTimeFunPtr)pvFun)
									{
										break;//原已经存在
									}
								}
