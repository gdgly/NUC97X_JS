...
BOOL _IsInitMMUTable = FALSE;//static BOOL _IsInitMMUTable = FALSE;
int _MMUMappingMode = MMU_DIRECT_MAPPING;//static int _MMUMappingMode = MMU_DIRECT_MAPPING;
...
int sysInitMMUTable(int cache_mode)
{
	...
	if (_IsInitMMUTable == FALSE)
	{
		...
		sysInitPageTable(0, 0, 0x00100000, cache_mode, MMU_DIRECT_MAPPING);
		...
		sysInitPageTable(0x80000000, 0x0, 0x00100000, CACHE_DISABLE, MMU_DIRECT_MAPPING);
