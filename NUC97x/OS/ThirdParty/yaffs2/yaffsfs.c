...
struct yaffsfs_DirSearchContxt yaffsfs_dsc[YAFFSFS_N_DSC];
struct yaffsfs_Inode yaffsfs_inode[YAFFSFS_N_HANDLES];
struct yaffsfs_FileDes yaffsfs_fd[YAFFSFS_N_HANDLES];
struct yaffsfs_Handle yaffsfs_handle[YAFFSFS_N_HANDLES];
...
// 		alt_path = kmalloc(path_length + 1, 0);
		alt_path = yaffs_malloc(path_length + 1);
		if (!alt_path)
			return -1;
        memset(alt_path, 0, path_length + 1);
...
int yaffs_open_sharing(const YCHAR *path, int oflag, int mode, int sharing)
{
	...
	else 
	{
		...
		if (obj) 
		{
			...
			if (!errorReported && writeRequested && obj->my_dev->read_only)
			{
				openDenied = 1;
