...
//RS485-1对RS485-2
	...
	if(RS485_BPS>=6)//波特率,O～11依次表示0=300,1=600,2=1200,3=2400,4=4800,5=7200,6=9600,7=19200,8=38400,9=57600,10=115200,11=230400,12=...
	{
		...
		MC(0,ADDR_BASE+(OFFSET_UART_Characteristics),1);
	}
	else
	{
		DisplayString(AutoDisplayRowCol->Row-1,16,0,(u8*)"\x0""FAIL");//显示字符串
		MC(1,ADDR_BASE+(OFFSET_UART_Characteristics),1);
...
//RS485-1对RS485-3
	...
	{
		DisplayString(AutoDisplayRowCol->Row-1,18,0,(u8*)"\x0""OK");//显示字符串
		MC(0,ADDR_BASE+(OFFSET_UART_Characteristics+2),1);
	}
	else
	{
		DisplayString(AutoDisplayRowCol->Row-1,16,0,(u8*)"\x0""FAIL");//显示字符串
		MC(1,ADDR_BASE+(OFFSET_UART_Characteristics+2),1);
		TestResult=1;//总测试结果0=合格,1=不合格,0xff=无结论
	}
#endif
...
IRError:
	...
	{
		DisplayString(AutoDisplayRowCol->Row-1,18,0,(u8*)"\x0""OK");//显示字符串
		MC(0,ADDR_BASE+(OFFSET_UART_Characteristics+1),1);
	}
	else
	{
		DisplayString(AutoDisplayRowCol->Row-1,16,0,(u8*)"\x0""FAIL");//显示字符串
		MC(1,ADDR_BASE+(OFFSET_UART_Characteristics+1),1);
		TestResult=1;//总测试结果0=合格,1=不合格,0xff=无结论
	}
