  ...
  u32 i;
    ...
    case 10://启动发送(先发7字节 0x55 CLA INS P1 P2 Len1 Len2)
		...
		case 11://启动接收
			...
			p16timer[0]=2000/10;//等待第1个0x55数据返回接收定时
			UART7Ctrl->Task++;
			//break;
		case 12://正在接收
			RxByte=0;
...
#if MainProtocol!=698
//	BR=5000000;
	TestResult=TEST_TESAM_CMD_TxRx(0x800E0002,0,0);//获取芯片序列号8字节
#else
//	BR=25000000;
	TestResult=TEST_TESAM_CMD_TxRx(0x803600FF,0,0);//获取芯片序列号8字节
	//---TESAM读写可靠性连继测试---
//	MR(ADDR_DATABUFF,ADDR_UART7Rx,0x56);
//	while(1)
//	{
//		TEST_TESAM_CMD_TxRx(0x803600FF,0,0);//获取芯片序列号8字节
//		if(DataComp(ADDR_DATABUFF,ADDR_UART7Rx,0x56)!=0)//数据比较,返回0=相等;当Byte>8时返回1=不相等;当Byte<=8时1=Data1>Data2,2=Data1<Data2
//		{
...
//	}
	...
	MC(TestResult,ADDR_BASE+(OFFSET_ESAM_Characteristics),1);
	return TestResult;
}
