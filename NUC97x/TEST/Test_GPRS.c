...
#include "../../STM32F4xx/DL698/DL698_DataDef.h"
#endif
#if MainProtocol==CSG
...
//					return;
		...
		case AT_CNUM://取本机号码
			...
			{//中兴CDMA(MC8332)
				TxByte=CopyString((u8 *)"AT+CNUM?\r",p8tx);//拷贝字符串,字符串ps拷贝到pd,返回拷贝字符串的字节数
			}
			break;		
		case AT_MYNETCON_APN://设置APN
			TxByte=CopyString((u8 *)p32[(4*AT_No)+1],p8tx);//拷贝字符串,字符串ps拷贝到pd,返回拷贝字符串的字节数
	#if MainProtocol==CSG
			CopyString((u8 *)"cmnet",(u8 *)ADDR_DATABUFF);//拷贝字符串,字符串ps拷贝到pd,返回拷贝字符串的字节数
			MC(0,ADDR_DATABUFF+5,1);
	#endif
	#if MainProtocol==376
		...
		case AT_MYNETSRV://设置服务参数
			switch(UART1Ctrl->SubTask)//子任务
			{
				case 0://确定工作模式
			#if MainProtocol==CSG
					i=1<<4;//B5-B4=0混合模式=1客户机模式=2服务器模式
					i|=0<<7;//B7=0 TCP, =1 UDP
			#endif
			#if MainProtocol==376
				...
				TCPClient:
					...
					break;
				case 2://TCP服务器模式
					TxByte=CopyString((u8 *)"AT$MYNETSRV=0,0,1,0,",p8tx);//拷贝字符串,字符串ps拷贝到pd,返回拷贝字符串的字节数
					p8tx[TxByte]='"';
					TxByte++;
...
#if MainProtocol==CSG
	Pin_Configure(PIN_GPRS_PCTRL,i);//ON输出=0
#endif
	//RI
}
...
*/
	switch(UART1Ctrl->LinkTask)//链接任务0-99初始过程,100=建立连接,101=登录,102=发退出登录,103=退出登录,104=关机
	{
		...
		case 2://开总电源
			OnR5V0();//打开R5V0电源
			Comm_Ram->UART1Timer = 201/10;//开总电源延时0.2秒
			UART1Ctrl->LinkTask++;//链接任务0-99初始过程,100=建立连接,101=登录,102=发退出登录,103=退出登录,104=关机
			return;//共用UART1Timer直接返回
		case 3://开模块ON/OFF输出=0
			...
			#if MainProtocol==CSG
				Pin_Configure(PIN_GPRS_PCTRL,//ON输出=0
								0|//B3-B0:复用功能选择AF(0-15)
...
//				UART1Ctrl->LinkTask++;//链接任务0-99初始过程,100=建立连接,101=登录,102=发退出登录,103=退出登录,104=关机
				...
				UART1Ctrl->LinkTask=7;//链接任务0-99初始过程,100=建立连接,101=登录,102=发退出登录,103=退出登录,104=关机					
			}
			return;//共用UART1Timer直接返回
		case 5://复位
			if(Comm_Ram->UART1Timer==0x0)
