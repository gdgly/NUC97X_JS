		...
		{//找到
			...
			{//抄表命令计数=0或9,上1次日冻结时间YYMMDDhhmm
				...
				for(Fn=161;Fn<168;Fn++)
				{
					...
					y=MRR(ADDR_AFN04F10+((y-1)*LEN_AFN04F10_Pn)+5,1);//通信协议类型 BIN 1
					if(y==21)
					{//上海规约目前只有F161
						break;
					}
...
u32 ReadMeter_Fn_YesNo(u32 PORTn,u32 ClassNo,u32 Pn,u8 * p8RMLib,u32 RMCount,u32 Protocol)//按大小类号抄表的Fn是否要抄;返回0=不抄,1=要抄;入口:ClassNo的D7-D4大类号D3-D0小类号,Pn,p8RMLib抄表库指针,RMCount抄表计数,Protocol(1=1997,其他=2007)
{
  ...
  UARTCtrl_TypeDef * UARTCtrl;
	...
	{//是485口
		...
		if(MRR(ADDR_AFN04F10+((i-1)*LEN_AFN04F10_Pn)+5,1)==30)
		{//DL/T645——2007
			p8=(u8*)(ADDR_AFN04F242);//抄表延时分钟数
			Fn=p8[0];
			if(Fn>=60)//抄表延时分钟数：1-60分钟
			{
				Fn=0x100;//1:00
			}
			else
			...
			if((i>=RMM_EndHM)||(i<Fn))
			{//RS485口在每日0点附近暂停抄表
				if(p8RMLib[4]&((1<<6)+(1<<3)))//数据分类:B6=补抄上2-3日冻结,B5=月冻结,B4=抄表日冻结,B3=日冻结,B2=曲线冻结,B1=小时冻结,B0=实时
				{//2类数据B6=补抄上2-3日冻结,B3=日冻结
					return 0;//不抄日月冻结数据
				}
			}
		}
			...
			{//重点户曲线
				...
				{//是重点户
					...
					if(i!=0)
					{
						...
						if((i!=0x02)&&(i!=0x52)&&(i!=0x62)&&(i!=0x72))
						{//大小类号=51或71,其他大小类号按51处理
							switch(RMCount)
							{
								case 0://电压曲线
								...
								case 1://当前正向有功总电能示值
									...
									return y;
								case 2://当前反向有功总电能示值
									//正向有功=正向有功+反向有功,当配置正向有功时也需抄反向有功
									y=Class2Data_Fn(ClassNo,101);//2类数据Fn配置,入口ClassNo的D7-D4大类号D3-D0小类号,Fn;返回0=不支持,1=支持
