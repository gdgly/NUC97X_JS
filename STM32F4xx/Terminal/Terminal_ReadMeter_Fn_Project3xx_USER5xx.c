...
u32 ReadMeter_Fn_YesNo(u32 PORTn,u32 ClassNo,u32 Pn,u8 * p8RMLib,u32 RMCount,u32 Protocol)//按大小类号抄表的Fn是否要抄;返回0=不抄,1=要抄;入口:ClassNo的D7-D4大类号D3-D0小类号,Pn,p8RMLib抄表库指针,RMCount抄表计数,Protocol(1=1997,其他=2007)
{
  ...
  UARTCtrl_TypeDef * UARTCtrl;
	...
	{
		if((UARTCtrl->RMTaskFlags&p8RMLib[4])==0)
		{
			return 0;//不是抄本类数据
		}
		...
		{//是485口
			...
			if(p8RMLib[4]&(1<<2))//数据分类:B6=补抄上2-3日冻结,B5=月冻结,B4=抄表日冻结,B3=日冻结,B2=曲线冻结,B1=小时冻结,B0=实时
			{//2类数据B6=补抄上2-3日冻结,B5=月冻结,B3=日冻结,B2=曲线冻结
				if((i>=RMM_EndHM)||(i<Fn))
				{//RS485口在每日0点附近暂停抄表
					return 0;//不抄曲线冻结数据
				}
		...
		{//找到
			if((RMCount!=0)&&(RMCount!=9))
			{
				...
				else
				{
					if(RMCount==10)
					{
						Fn=185;//正向有功最大需量及发生时间
					}
					else
					{
						Fn=187;//反向有功最大需量及发生时间
					}
				}
