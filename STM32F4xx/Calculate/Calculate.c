...
//年年(先高字节后低字节),月,日,时,分,秒	
	...
	while(1)
	{
		...
		{//是2月
			...
			if(d==29)
			{
				...
				{
					S-=24*60*60;
					y=YYMD_Add1D_hex(YYMDHMS>>16);//年年月日加1日,返回加1后的年年月日
					d=YYMDHMS&0xffff;
					YYMDHMS=y;
					YYMDHMS<<=16;
					YYMDHMS|=d;
		...
		else
		{
			...
			{//能被4整除的年
				...
				if(m>2)
				{
					y=YYMD_Sub1D_hex(YYMDHMS>>16);//年年月日减1日,返回减1后的年年月日
					d=YYMDHMS&0xffff;
					YYMDHMS=y;
					YYMDHMS<<=16;
					YYMDHMS|=d;
	...
	while(1)
	{
		...
		{
			S-=i;
			m=YYM_AddM_hex(YYMDHMS>>24,1);//年年月加m月,返回加m月后的年年月
			m<<=8;
			m+=1;
			d=YYMDHMS&0xffff;
			YYMDHMS=m;
