...
//版本-------------------------------------------------------------------------------------
#define I0_UpDataInit     3//I0软件更新初始化:0=否,1=硬件初始化,2=数据区初始化,3=参数及全体数据区初始化(即恢复至出厂配置),4=参数(除与系统主站通信有关的)及全体数据区初始化,5=参数及全体数据区初始化(即恢复至出厂配置)后硬件复位,6=仅部份参数初始化
#define I1_UpDataInit     0//I1软件更新初始化:0=否,1=硬件初始化,2=数据区初始化,3=参数及全体数据区初始化(即恢复至出厂配置),4=参数(除与系统主站通信有关的)及全体数据区初始化,5=参数及全体数据区初始化(即恢复至出厂配置)后硬件复位,6=仅部份参数初始化
#define I2_UpDataInit     0//I2软件更新初始化:0=否,1=硬件初始化,2=数据区初始化,3=参数及全体数据区初始化(即恢复至出厂配置),4=参数(除与系统主站通信有关的)及全体数据区初始化,5=参数及全体数据区初始化(即恢复至出厂配置)后硬件复位,6=仅部份参数初始化
#define E1_UpDataMeterInit     3//E1软件更新仪表初始化:0=否,1=硬件初始化,2=数据区初始化,3=参数及全体数据区初始化(即恢复至出厂配置),4=参数(除与系统主站通信有关的)及全体数据区初始化,5=参数及全体数据区初始化(即恢复至出厂配置)后硬件复位,6=仅部份参数初始化
#define E1_UpDataTerminalInit     3//E1软件更新终端初始化:0=否,1=硬件初始化,2=数据区初始化,3=参数及全体数据区初始化(即恢复至出厂配置),4=参数(除与系统主站通信有关的)及全体数据区初始化,5=参数及全体数据区初始化(即恢复至出厂配置)后硬件复位,6=仅部份参数初始化
...
#define E1_Stop     0//E1运行需比较软件版本号是否相同:0=否,1=是(当版本号与I2不相同时停止运行)
#define E2_Stop     0//E2运行需比较软件版本号是否相同:0=否,1=是(当版本号与I2不相同时停止运行)
...
#define SVDATE     0x130829//软件发行日期:年月日
#define HVER     0x100//硬件版本号:1.00
#define HVDATE     0x130108//硬件发行日期:年月日
...
#define RS485_1PORT     2//RS485_1通信口号(Ethernet专用RAM区)
#define RS485_2PORT     3//RS485-2通信口号(Ethernet专用RAM区)
#define IRPORT     4//红外通信口号
#define RS485_3PORT     5//RS485_3通信口号
#define RS485_4PORT     6//RS485_4通信口号(载波)
...
//UART0(RS232)
#define LEN_UART0IntRx     256//UART0中断接收缓冲长度
#define LEN_UART0Ctrl     200//UART0控制数据长度
#define LEN_UART0Rx     512//UART0接收缓冲长度
#define LEN_UART0Tx     256//UART0发送缓冲长度
#define LEN_UART0FnDataBuff     0//UART0抄表Fn数据缓冲长度
//UART1(GPRS)
#define LEN_UART1IntATRx     1024//UART1中断接收缓冲长度AT命令
#define LEN_UART1IntRx     1024//UART1中断接收缓冲长度
...
//UART2(RS485-1)(Ethernet专用RAM区)
#define LEN_UART2IntRx     256//UART2中断接收缓冲长度
#define LEN_UART2Ctrl     200//UART2控制数据长度
#define LEN_UART2Rx     2048//UART2接收缓冲长度
#define LEN_UART2Tx     256//UART2发送缓冲长度
#define LEN_UART2FnDataBuff     256//UART2抄表Fn数据缓冲长度
//UART3(RS485-2)(Ethernet专用RAM区)
#define LEN_UART3IntRx     256//UART3中断接收缓冲长度
#define LEN_UART3Ctrl     200//UART3控制数据长度
#define LEN_UART3Rx     2048//UART3接收缓冲长度
#define LEN_UART3Tx     256//UART3发送缓冲长度
...
#define LEN_UART4FnDataBuff     0//UART4抄表Fn数据缓冲长度
//UART5(RS485-3)
...
//UART6(RS485-4)
#define LEN_UART6IntRx     8//UART6中断接收缓冲长度
#define LEN_UART6Ctrl     0//UART6控制数据长度
#define LEN_UART6Rx     0//UART6接收缓冲长度
#define LEN_UART6Tx     0//UART6发送缓冲长度
...
//UART8(电表ESAM)
#define LEN_UART8IntRx     (6+256+2)//UART8中断接收缓冲长度
#define LEN_UART8Ctrl     200//UART8控制数据长度
#define LEN_UART8Rx     0//UART8接收缓冲长度
#define LEN_UART8Tx     256//UART8发送缓冲长度
#define LEN_UART8FnDataBuff     0//UART8抄表Fn数据缓冲长度
//UART9(电表CPUCARD)
#define LEN_UART9IntRx     (6+256+2)//UART9中断接收缓冲长度
#define LEN_UART9Ctrl     200//UART9控制数据长度
#define LEN_UART9Rx     0//UART9接收缓冲长度
#define LEN_UART9Tx     256//UART9发送缓冲长度
#define LEN_UART9FnDataBuff     0//UART9抄表Fn数据缓冲长度
//UART10(Ethernet)
#define LEN_UART10IntRx     8//8192//UART10中断接收缓冲长度
#define LEN_UART10Ctrl     0//64//UART10控制数据长度
#define LEN_UART10Rx     0//8192//UART10接收缓冲长度
#define LEN_UART10Tx     0//8192//UART10发送缓冲长度
...
#define MaxMeterCurveCongeal     (60*24*40)//最大曲线点数,60的整数倍(搜索时跳越用)
...
#define PowerOnRMTask     0//电源开抄表任务;0=没有,1=有
...
#define MaxRS485User     1//最大RS485抄表数
#define MaxKeyUser     0//最大集中抄表重点用户数
#define MaxCarrierUser     0//最大载波抄表数
...
#define MaxTotalAddGroup     1//8//最大总加组数
...
#define MaxCurveCongeal     (96*30)//最大曲线点数
...
#define EventProject     0//事件记录采集方案:0=原能走台体标准,1=按新智能电能表及采集终端事件记录采集规则
...
#define WaitHeartberatACKMaxTimer     600//心跳发出等待主站确认最大定时秒
...
#define SoftBug1     1//0=正常功能,1==376主站测试软件BUG.小时冻结密度
...
//#define Extern_CDMA_FTP_RxTx     0//外部程序:0=I2,1=E1,2=E2,3=;CDMA FTP文件下载
...
#define IC_SST39VF6401B     0//0=没,1=有
#define IC_S29GL01G     0//0=没,1=有
#define IC_S29GL512     0//0=没,1=有
