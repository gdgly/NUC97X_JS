...
#define ADDR_SYSTEM     (ADDR_FilePlay+64)
  ...
  u8 Init_OS;//OS需重启原因标志:B0=软件更新,B1=FS,B2=USB0,B3=USB1,B4=ETH,B5=HardFault,B6=,B7=TEST
	u8 After_Init_OS;//OS重启后使用标志:B0=软件更新,B1=FS,B2=USB0,B3=USB1,B4=ETH,B5=HardFault,B6=,B7=TEST
  u8 MS_Task;//MS任务
  u8 IntFlags;//中断服务冲突标志
					 ...
					 //B3=中断中读写EFLASH冲突标志
					 //
	u16 CurrentlyWirelessConnectType;//麻晓方用
	u16 res3;
	u8 msFILEchange;//文件管理数据变化:0=变化,!=没变化
...
#define ADDR_TYMDHMS     (ADDR_MYMDHMS+8)//终端时钟
  u8 TYMDHMS[10];//秒,分,时,日,月,年,周(0-6),NC
#define ADDR_URTC     (ADDR_TYMDHMS+10)//时钟电池电压(内部)(半字对齐)
  u16 URTC;//BCD 2位小数
...
#define ADDR_Comm_Ram_End     (ADDR_RUNSPEEDTEST+4+1)
