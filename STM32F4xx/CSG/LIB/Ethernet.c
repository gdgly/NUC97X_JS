			...
			if(p16timer[0]==0x0)
			{
				...
				else
				{
					...
					{//第二次判断
						...
						WarningOccur(ENETPORT,(u8 *)"以太网电缆被拔出");//告警发生,入口告警代码和告警字符串
					}
				}
			}
			return;
			...
			switch(UARTCtrl->LinkSubTask)//链接子任务
			{
				case 0://发起连接
					...
					{//TCP
						...
						{//主IP 南网上行协议设置的 IP 地址格式与以太网芯片需要地址格式高低字节相反，因此需要倒序
						...
						if(UARTCtrl->LinkCount >= 4)
						{
							...
							//WarningOccur(ENETPORT,(u8 *)"以太网无可用主站参数");
							return;
						}
		...
		case 6://多帧数据正在发送
			...
			else
			{
				...
				if(UARTCtrl->LinkTask != 100)
				{
					...
					if(Terminal_Ram->ENETHeartBeat_S_Timer == 0) Terminal_Ram->ENETHeartBeat_S_Timer = 15*60;//err20180228-5 添加0限制条件
				}
			}
			break;
	}
