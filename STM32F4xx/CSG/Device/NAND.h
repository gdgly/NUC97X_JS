...
void BlockErase_NAND(u32 ADDR_Dest,u32 Busy_Wait);//NAND_FLASH 块擦除;没块替换;入口:Busy_Wait=1NAND忙时等待Busy_Wait=0不等待
...
void Check_4kNandBuff(void);//检查4K(BKPSRAM) NAND FLASH的写缓冲,若无效则清0;
...
void Check_128kNandBuff(void);//检查128K(BKPSDRAM) NAND FLASH的写缓冲,若无效则清0
