...
//#include "Test_ReadMeter_3762_Std.h"
//#include "Test_ReadMeter_3762_NoRouter.h"
...
#if (Project/100)==2 || (Project/100)==9//方案0=智能表,100=网络表,200=集中器,300=专变终端,400=公变终端,500=集中器Ⅱ型,600=通信模块,700=
...
//	u32 * p32;
	...
	{//模块状态
		...
		case 6://回确认帧(主动上报厂商代码的响应,发确认帧停止)
			i=Terminal_Router->RouterInitSubTask;
			switch(i)
			{
				case 0:
					i=p8rxbuff[13];
		...
		case 8://读本地模块运行模式
			switch(Terminal_Router->RouterInitSubTask)
			{
				...
				case 2:
				#if (USER/100)==14//福建
					Uart_3762_TxData(PORTn,0x53,2,0);//376.2发送数据填入发送区;入口时若有信息域目的地址数据单元等则已填入且数据单元暂固定填入发送缓冲区+128地址
				#else
