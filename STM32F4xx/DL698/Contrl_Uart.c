			...
			{//接收数据
				...
				else
				{
					if(RxByte >= 7)
					{
						...
						{//接收完成
							//和校验计算
							//i=RAMCRC16((u32)p8rx,4);//RAM中数据CRC16计算(硬件算法)
							i=RAMCRC16_Soft((u32)p8rx,4);//RAM中数据CRC16计算(软件算法)
							if(i != (p8rx[4]+(p8rx[5]<<8)))
							{//和校验错误
								RxByte = 0;
								continue;
							}
...
//控制通信任务
	...
	switch(UARTCtrl->SubTask)
	{
		...
		TX:
			p8tx[0]=0x68;
			//i=RAMCRC16((u32)p8tx,4);//RAM中数据CRC16计算(硬件算法)
			i=RAMCRC16_Soft((u32)p8tx,4);//RAM中数据CRC16计算(软件算法)
			p8tx[4]=i;
			p8tx[5]=i>>8;
			p8tx[6]=0x16;
		...
		case 1:
			if(UARTCtrl->Task==2)
			{//接收超时(或错误)
				Comm_Ram->MODE3in1=1;//三合一终端当前运行模式:0=专变,1=集中器
...
//			}
		...
		case 9://文件发送帧
			p8tx[0]=0x68;
			p8tx[1]=0x51;//升级程序帧
			p8tx[2]=UARTCtrl->TxSEQ;//已发帧数
			p8tx[3]=0;
			MR(((u32)p8tx+4),ADDR_Download_ENTRY+(256*UARTCtrl->TxSEQ),256);
			//i=RAMCRC16((u32)p8tx,4+256);//RAM中数据CRC16计算(硬件算法)
			i=RAMCRC16_Soft((u32)p8tx,4+256);//RAM中数据CRC16计算(软件算法)
			p8tx[4+256]=i;
			p8tx[5+256]=i>>8;
			p8tx[6+256]=0x16;
