...
//#define LENmax_F206_4     (2+(10*NUMmax_F206_4))
	...
	while(n--)
	{
		if(p8[4]==0xff)//FFH表示无效
		{
			p8[4]=0;
		}
		if(p8[5]==0xff)//FFH表示无效
		{
			p8[5]=0;
		}
		hms1=(p8[3]<<16)|(p8[4]<<8)|p8[5];
		if(p8[8]==0xff)//FFH表示无效
		{
			p8[8]=0;
		}
		if(p8[9]==0xff)//FFH表示无效
		{
			p8[9]=0;
		}
...
//功控,电控LED指示
	MR(ADDR_DATABUFF,ADDR_8103_3,(ADDR_8108_3+LENmax_8108_3)-(ADDR_8103_3));//投入(总加组号)
//功控LED指示
	//时段功控投入(总加组号)
	//厂休功控投入(总加组号)
	//营业报停功控投入(总加组号)
...
//跳闸红和绿LED 指示
	x=0;//功控轮次设定,b0-b7
	MR(ADDR_DATABUFF,ADDR_230x_14,LENmax_230x_14);//功控轮次设定(总加组号)
	p8=(u8 *)(ADDR_DATABUFF);
	x=0;
	...
	MR(ADDR_DATABUFF,ADDR_230x_15,LENmax_230x_15);//电控轮次设定(总加组号)
	p8=(u8 *)(ADDR_DATABUFF);
	y=0;//电控轮次设定,b0-b7
	for(i=0;i<NUMmax_TAG;i++)
	{
...
//终端报警
	x=0;//报警标志;11=催费告警声,12=允许合闸声,13=功控告警声,14=电控告警声,15=遥控跳闸
	//催费告警
	p8=(u8 *)(ADDR_8002_2);//催费告警状态，只读 ∷=enum{未告警(0)，告警(1)}
	p8[0]=DataType_enum;
	p8[1]=0;
