...
#define Init_Report_Event_3030     1//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端初始化事件                   
...
#define Init_Report_Event_3104     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表时钟超差事件               
...
#else//#if USER!=1500//1500=河电科院台体测试版(仅终端事件设置参数同国网电科院测试版)
...
#endif//#if USER!=1501//1501=安徽过电科院台体测试版(仅终端事件设置参数同国网电科院测试版)
