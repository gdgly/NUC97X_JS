...
#if USER!=1700//1701=江苏版
//电能表失压事件
#define Init_Enable_Event_3000     1//事件有效标识bool
#define Init_Report_Event_3000     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表欠压事件                   
#define Init_Enable_Event_3001     1//事件有效标识bool
#define Init_Report_Event_3001     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表过压事件                   
#define Init_Enable_Event_3002     1//事件有效标识bool
#define Init_Report_Event_3002     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表断相事件                   
#define Init_Enable_Event_3003     1//事件有效标识bool
#define Init_Report_Event_3003     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表失流事件                   
#define Init_Enable_Event_3004     0//事件有效标识bool
#define Init_Report_Event_3004     0//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表过流事件                   
#define Init_Enable_Event_3005     1//事件有效标识bool
#define Init_Report_Event_3005     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表断流事件                   
#define Init_Enable_Event_3006     1//事件有效标识bool
#define Init_Report_Event_3006     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表潮流反向事件               
...
#define Init_Report_Event_300F     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表电流逆相序事件             
#define Init_Enable_Event_3010     1//事件有效标识bool
#define Init_Report_Event_3010     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表掉电事件                   
#define Init_Enable_Event_3011     1//事件有效标识bool
#define Init_Report_Event_3011     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表编程事件                   
...
//电能表电压不平衡事件             
#define Init_Enable_Event_301D     1//事件有效标识bool
#define Init_Report_Event_301D     0//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表电流不平衡事件             
#define Init_Enable_Event_301E     1//事件有效标识bool
...
//电能表时钟故障事件               
#define Init_Enable_Event_302E     1//事件有效标识bool
#define Init_Report_Event_302E     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表计量芯片故障事件           
#define Init_Enable_Event_302F     0//事件有效标识bool
#define Init_Report_Event_302F     0//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//通信模块变更事件                 
#define Init_Enable_Event_3030     1//事件有效标识bool
#define Init_Report_Event_3030     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端初始化事件                   
#define Init_Enable_Event_3100     1//事件有效标识bool
#define Init_Report_Event_3100     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端版本变更事件                 
#define Init_Enable_Event_3101     1//事件有效标识bool
#define Init_Report_Event_3101     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端状态量变位事件               
#define Init_Enable_Event_3104     1//事件有效标识bool
#define Init_Report_Event_3104     0//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表时钟超差事件               
#define Init_Enable_Event_3105     1//事件有效标识bool
#define Init_Report_Event_3105     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端停/上电事件                  
#define Init_Enable_Event_3106     1//事件有效标识bool
#define Init_Report_Event_3106     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端直流模拟量越上限事件         
...
//设备故障记录                     
#define Init_Enable_Event_310A     1//事件有效标识bool
#define Init_Report_Event_310A     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表示度下降事件               
#define Init_Enable_Event_310B     1//事件有效标识bool
#define Init_Report_Event_310B     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能量超差事件                   
#define Init_Enable_Event_310C     0//事件有效标识bool
#define Init_Report_Event_310C     0//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表飞走事件                   
#define Init_Enable_Event_310D     1//事件有效标识bool
#define Init_Report_Event_310D     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表停走事件                   
#define Init_Enable_Event_310E     1//事件有效标识bool
#define Init_Report_Event_310E     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端抄表失败事件                 
#define Init_Enable_Event_310F     1//事件有效标识bool
#define Init_Report_Event_310F     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//月通信流量超限事件               
#define Init_Enable_Event_3110     0//事件有效标识bool
#define Init_Report_Event_3110     0//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//发现未知电能表事件               
#define Init_Enable_Event_3111     1//事件有效标识bool
#define Init_Report_Event_3111     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//跨台区电能表事件                 
#define Init_Enable_Event_3112     1//事件有效标识bool
#define Init_Report_Event_3112     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端对时事件                     
#define Init_Enable_Event_3114     1//事件有效标识bool
...
//输出回路接入状态变位事件记录     
#define Init_Enable_Event_3117     1//事件有效标识bool
#define Init_Report_Event_3117     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端编程记录                     
#define Init_Enable_Event_3118     1//事件有效标识bool
#define Init_Report_Event_3118     0//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//终端电流回路异常事件             
#define Init_Enable_Event_3119     1//事件有效标识bool
#define Init_Report_Event_3119     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//电能表在网状态切换事件           
...
//拒绝从节点入网记录           
#define Init_Enable_Event_311F     1//事件有效标识bool
#define Init_Report_Event_311F     3//不上报（0）,事件发生上报（1），事件恢复上报（2），事件发生恢复均上报（3）
//功控跳闸记录                     
...
#else//#if USER!=1700//1700=电科院台体测试版(仅终端事件设置参数同国网电科院测试版)
...
#endif//#if USER!=1701//1701=江苏过电科院台体测试版(仅终端事件设置参数同国网电科院测试版)
...
//  电压触发上限  long-unsigned（单位：V，换算：-1），
	DataType_long_unsigned,
//	0x06,0xB4,//78%参比电压
	((MeterUBhex*80/100)>>8)&0xff,((MeterUBhex*80+50)/100)&0xff,//78%参比电压
//  判定延时时间  unsigned（单位：s，换算：0）
	DataType_unsigned,
	60,
//}
...
//关联对象属性表∷=array OAD
	...
	13,
	DataType_OAD,
	0x00,0x10,0x02,0x01,//00100201——当前正向有功总电能示值
	DataType_OAD,
	0x00,0x20,0x02,0x01,//00200201——当前反向有功总电能示值
	DataType_OAD,
	0x00,0x50,0x02,0x01,//00500201——当前一象限无功总电能示值
	DataType_OAD,
	0x00,0x60,0x02,0x01,//00600201——当前二象限无功总电能示值
	DataType_OAD,
	0x00,0x70,0x02,0x01,//00700201——当前三象限无功总电能示值
	DataType_OAD,
	0x00,0x80,0x02,0x01,//00800201——当前四象限无功总电能示值
	DataType_OAD,
	0x20,0x00,0x02,0x00,//20000200——当前三相电压
	DataType_OAD,
	0x20,0x01,0x02,0x00,//20010200——当前三相电流
	DataType_OAD,
	0x20,0x04,0x02,0x00,//20040200——当前总及三相有功功率
	DataType_OAD,
	0x20,0x05,0x02,0x00,//20050200——当前总及三相无功功率
	DataType_OAD,
	0x20,0x0A,0x02,0x00,//200A0200——当前总及三相功率因数
	DataType_OAD,
	0x20,0x14,0x02,0x00,//20140200——电能表运行状态字
	DataType_OAD,
	0xF2,0x03,0x02,0x00,//F2030200——终端状态量标志（D0~D7对应1~8路状态量的状态ST）
};
...
const u8 InitData_311F[]=//拒绝从节点入网记录
{
//有效标识∷=bool
	DataType_bool,
	Init_Enable_Event_311F,
//上报标识∷=enum
//{
//不上报（0），
//事件发生上报（1），
...
//}
	DataType_enum,
	Init_Report_Event_311F,
//关联对象属性表∷=array OAD
	DataType_array,
	0,
};
const u8 InitData_311F_6[]=//拒绝从节点入网记录-6
{
//属性6（配置参数）∷=structure
	DataType_structure,
	0,
