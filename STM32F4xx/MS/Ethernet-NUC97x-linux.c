...
#define DEBUG_ETH 0//0,B不开启ETH调试，1开启ETH调试
...
void Ethernet_RxTx(void)
{
  ...
  UARTCtrl=(UARTCtrl_TypeDef *)(Get_ADDR_UARTnCtrl(ENETPORT));
	...
	if(UARTCtrl->LinkTask<101)//101://登录ok
	{
		...
		{
			ms->Pin_OutH(PIN_ENET_LINK_LED);//引脚输出低,入口引脚使用名
		}
	}
	...
	switch(UARTCtrl->LinkTask)
	{
		case 0://0=延时关机
			...
			return;
		case 1://1=关电源
			if(UARTCtrl->TCPSocket>=3)//进入系统时，该变量为0，为系统基本输入输出用描述符，不可用于socket通讯
			{
			#if ((DEBUG==1)&&(DEBUG_ETH==1))
			...
			if(UARTCtrl->UDPSocket>=3)//进入系统时，该变量为0，为系统基本输入输出用描述符，不可用于socket通讯
			{
			#if ((DEBUG==1)&&(DEBUG_ETH==1))
			...
			WarningOccur(ENETPORT,(u8 *)"重启以太网连接");//告警发生,入口告警代码和告警字符串
...
//			#endif
		...
		case 2://开电源,ETH系统初始化
			...
			if(p16timer[0]!=0x0)
			{
				...
				{
				#if RJ45SELECT!=1//以太网RJ45通道选择:0=eth0(IP101),1=eth1(LAN8720),2=eth0和eth1二个独立口,eth0用于以太网RJ45通道
					if(ms->Pin_Read(PIN_ENE5V0)==0)//读引脚,入口引脚使用名;返回0或1	
					{
...
//						OnE5V0();//打开E5V0电源
				...
				#else
					if(ms->Pin_Read(PIN_EN2E5V0)==0)//读引脚,入口引脚使用名;返回0或1	
					{
...
//						On2E5V0();//打开E5V0电源
		...
		ms->link_printf("UARTCtrl->LinkTask=%d\n",UARTCtrl->LinkTask,0,0);
			...
			p16timer[0]=5000/10;//等待电缆RJ45插入2s延时
        ...
        if(fd>=0)
				{
					...
					ms->link_read(fd,&i,1);//以太网插入为'1'，没插入为'0'
				}
				ms->link_close(fd);
//				CopyString((u8 *)"标志：  ",(u8*)ADDR_DATABUFF);//拷贝字符串;返回拷贝字符串字节数(不计结束0)
//				MR(ADDR_DATABUFF+6,(u32)&i,4);
//				WarningOccur(ENETPORT,(u8 *)ADDR_DATABUFF);//告警发生,入口告警代码和告警字符
//			#if RJ45SELECT!=1//以太网RJ45通道选择:0=eth0(IP101),1=eth1(LAN8720),2=eth0和eth1二个独立口,eth0用于以太网RJ45通道
//				ms->Disable_os();//暂停os
...
//			#endif
				...
				{//RJ45插入同时自动自检测完成(Link status and Auto-negotiation complete)
					...
					WarningOccur(ENETPORT,(u8 *)"以太网电缆已插入");//告警发生,入口告警代码和告警字符串
					Terminal_Ram->ENETSEL=0;//以太网模块选择0=,1=,2=,3=RJ45没插入1次,4=没插入2次,5=没插入5次,6=
				#if MainProtocol==376
				...
				else
				{
					...
					{//第二次判断
						if(Terminal_Ram->ENETSEL<8)//以太网模块选择0=,1=,2=,3=RJ45没插入1次,4=没插入2次,5=没插入5次,6=
						{
...
//							#warning "UARTCtrl->LinkTask++;"
						...
						WarningOccur(ENETPORT,(u8 *)"检测以太网电缆状态");//告警发生,入口告警代码和告警字符串
					}
				}
			}
			return;
...
//					p8tx=StrCopy(p8tx,(char*)ADDR_IP4GATEWAY_ascii);
					...
					WarningOccur(ENETPORT,(u8 *)"配置以太网地址");//告警发生,入口告警代码和告警字符串
...
//					}
		...
		ms->link_printf("eth_ms->link_socket_return/UARTCtrl->TCPSocket=%d\n",UARTCtrl->TCPSocket,0,0);
#endif
						if(UARTCtrl->TCPSocket<3)//0,1,2为系统基本输入输出用描述符，不可用于socket通讯
						{
							return;
						}
						i=ms->link_fcntl(UARTCtrl->TCPSocket,F_GETFL,0);//获取文件描述符的属性
						ms->link_fcntl(UARTCtrl->TCPSocket, F_SETFL, i | O_NONBLOCK);	//将文件描述符的属性设置为非阻塞
						...
						i=IP_ASCII_HEX(ADDR_IP4ADDR_ascii-1);//ASCII转为HEX的IP;入口可为指向非ASCII数字0-9的地址;返回0表示非IP地址
...
#if RJ45SELECT==0//以太网RJ45通道选择:0=eth0(IP101),1=eth1(LAN8720),2=eth0和eth1二个独立口,eth0用于以太网RJ45通道
			CopyString((u8 *)"ETH0: ",(u8*)ADDR_DATABUFF);//拷贝字符串;返回拷贝字符串字节数(不计结束0)
#elif RJ45SELECT==1
			CopyString((u8 *)"ETH1: ",(u8*)ADDR_DATABUFF);//拷贝字符串;返回拷贝字符串字节数(不计结束0)
#elif RJ45SELECT==2
			CopyString((u8 *)"ETH0: ",(u8*)ADDR_DATABUFF);//拷贝字符串;返回拷贝字符串字节数(不计结束0)
#endif						
    ...
    ms->link_printf("addr.sin_addr.s_addr=%x\n",addr.sin_addr.s_addr,0,0);
		...
		ms->link_printf("eth_ms->link_errno_return=%d\n",i,0,0);
						...
						UARTCtrl->LinkSubTask=1;//等等TCP连接
						p16timer[0]=1000/10;
						return;
/*						
					#if MainProtocol==376
...
*/
					...
					{//UDP
						...
						if(UARTCtrl->UDPSocket<3)//0,1,2为系统基本输入输出用描述符，不可用于socket通讯
						{
							return;
						}
						addr.sin_family = AF_INET;// 使用 IPv4 协议
...
//						}
		...
		ms->link_printf("eth_ms->link_errno=%d\n",i,0,0);
							...
							{//还正在连接
								p16timer[0]=1000/10;
								WarningOccur(ENETPORT,(u8 *)"以太网正在连接主站");//告警发生,入口告警代码和告警字符
								return;
							}
							else if(i==EALREADY)
							{//
			...
			else if(i<0)//非阻塞模式下的接收
			{
			  ...
			  if((errno!=EINTR)&&(errno!=EAGAIN))
				...
				}else if(errno==ECONNRESET)
				{
					ms->link_close(UARTCtrl->TCPSocket);
				  UARTCtrl->TCPSocket=0;
				  UARTCtrl->LinkTask=0;//á′?óè???0-993?ê?1y3ì,100=?¨á￠á??ó,101=μ???,102=·￠í?3?μ???,103=í?3?μ???,104=1??ú
				}
				return;
			}
			if(i<=LEN_UARTnTx)
