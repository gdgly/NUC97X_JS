...
const u8 TEXT_ERRORIC5[]={"IC_ATT7022"};
#else
...
const u8 TEXT_ERRORIC7[]={"IC_IP101"};
	#else
...
const u8 TEXT_ERRORIC14[]={"IR"};//红外通信(自动,非仿真口用自发自收检测,IO仿真口在中断中检测)
...
//硬件错误
	...
	for(i=0;i<32;i++)
	{
		...
		if(i==14)
		{
			continue;//B14=红外通信(自动,非仿真口用自发自收检测,IO仿真口在中断中检测)
		}
	#endif
		if(Err&0x1)
		{
