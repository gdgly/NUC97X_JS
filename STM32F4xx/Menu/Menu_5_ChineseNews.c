...
void Menu_5_1(void)//菜单5_1
{
	...
	for(j=0;j<2;j++)
	{
		...
		{
			m=MRR(ADDR_AFN05F32+202*(MenuCount->v2-1),1);
			if(m>>4==0 && (m&0xf)==MenuCount->v2-1)
			{//判断是否为普通信息0，信息编号是否为MenuCount->v2-1
				y=MRR(ADDR_AFN05F32+202*(MenuCount->v2-1)+1,1);//读普通信息长度
				p8=(u8*)ADDR_DATABUFF;
				MC(0,ADDR_DATABUFF,202);
				MR(ADDR_DATABUFF,ADDR_AFN05F32+202*(MenuCount->v2-1)+2,y);
				x=DisplayString1(3,0,0,p8+MenuCount->v1);
				Ln=x&0xff;
				i=x>>16;
				if(Ln==8 && p8[i]!=0)
				{
				...
				p16=(u16 *)(ADDR_STRINGBUFF+4+1*84+18*2);
...
void Menu_5_2(void)//菜单5_2
{
	...
	for(j=0;j<2;j++)
	{
		...
		if(MenuCount->v2<=16 && MenuCount->v2>=1)
		{
			m=MRR(ADDR_AFN05F32+202*16+202*(MenuCount->v2-1),1);
			if(m>>4==1 && (m&0xf)==MenuCount->v2-1)//modified by sym 20120504
			{//判断是否为重要信息1，信息编号是否为MenuCount->v2-1
				y=MRR(ADDR_AFN05F32+202*16+202*(MenuCount->v2-1)+1,1);//读重要信息长度
				p8=(u8*)ADDR_DATABUFF;
				MC(0,ADDR_DATABUFF,202);
				MR(ADDR_DATABUFF,ADDR_AFN05F32+202*16+202*(MenuCount->v2-1)+2,y);
				x=DisplayString1(3,0,0,p8+MenuCount->v1);
				Ln=x&0xff;
				i=x>>16;
				if(Ln==8 && p8[i]!=0)
