...
void Terminal_ReadMeter_3762_CHINT(u32 PORTn)//ÕıÌ©ÔØ²¨Ä£¿é
{
	u32 i;
	u32 x;
	u32 y;
...
//	p16timer=(u16 *)Get_ADDR_UARTnTimer(PORTn);
	...
	switch(Terminal_Router->RouterInitTask)//Â·ÓÉÆ÷³­¶ÁÈÎÎñ0-99¹«¹²³õÊ¼»¯,100-199·Ö³§¼Ò³õÊ¼»¯,>=200Â·ÓÉ³­±í
	{
		case 100://²éÉ¾³ı
			...
			{
				case 0://ÇåÉ¾³ı»òÌí¼Ó½ÚµãÊı¼ÆÊı
					Terminal_Router->TempCount=0;//É¾³ı»òÌí¼Ó½ÚµãÊı¼ÆÊı
					Terminal_Router->RouterInitSubTask++;//×ÓÈÎÎñ
					break;
				case 1://²éÉ¾³ı
					for(i=0;i<100;i++)
					{
						...
						for(x=0;x<AFN04F10MaxPn;x++)
						{
							...
							{//µØÖ·ÏàÍ¬
								...
								{//²âÁ¿µãÓĞĞ§
									...
									{//¶Ë¿ÚºÅ31
										MEMWriteReg(x+1,(u32)&Terminal_Note->List[Terminal_Router->TempCount].MeterNo,2);//¶ÔÓ¦µçÄÜ±í/½»Á÷²ÉÑù×°ÖÃĞòºÅ(´Ó1¿ªÊ¼)
										break;
									}
								}
							}
						...
						if(x==AFN04F10MaxPn)
						{//É¾³ı
							x=(Terminal_Router->NoteNum-(Terminal_Router->TempCount+1))*LEN_NoteList;//´ÓµØÖ·±íÖĞÉ¾È¥
						...
						{//²âÁ¿µãÓĞĞ§
							...
							{//¶Ë¿ÚºÅ31
								...
								{//Ìí¼Ó
									...
									Terminal_Note->List[Terminal_Router->NoteNum].MeterNo=Terminal_Router->TempCount+1;//¶ÔÓ¦µçÄÜ±í/½»Á÷²ÉÑù×°ÖÃĞòºÅ(´Ó1¿ªÊ¼)
									Terminal_Router->NoteNum++;
								}
							}
						}
...
//			{
//				UpData_CHINT(PORTn);//Â·ÓÉÈí¼ş¸üĞÂ
//				break;
//			}
			...
			{
				GeneralReadMeter_3762_CHINT(PORTn);//ÔØ²¨Ä£¿é,³£¹æ³­±í
			}
			else
			{
				KeyUserReadMeter_3762_CHINT(PORTn);//ÔØ²¨Ä£¿é,ÖØµã»§³­±í
			}
			break;
		default:
			Terminal_Router->RouterInitTask=0;//Â·ÓÉÆ÷³­¶ÁÈÎÎñ0-99¹«¹²³õÊ¼»¯,100-199·Ö³§¼Ò³õÊ¼»¯,>=200Â·ÓÉ³­±í
...
void GeneralReadMeter_3762_CHINT(u32 PORTn)//ÔØ²¨Ä£¿é,³£¹æ³­±í
{
	u32 i;
	u32 x;
	u32 y;
...
void KeyUserReadMeter_3762_CHINT(u32 PORTn)//ÔØ²¨Ä£¿é,ÖØµã»§³­±í
{
	u32 i;
	u32 x;
	u32 y;
...
void UpData_CHINT(u32 PORTn)//Â·ÓÉÈí¼ş¸üĞÂ
{
