...
//初始化
	switch(UARTCtrl->LinkTask)
	{
		case 0://上电或复位
			...
			Pin_CFG(PIN_ENCtrlModulOut,0+(1<<5)+(1<<17)+(0<<18));//关控制模块输出允许
			if(i<0x0010)
			{//是小于0时10分,关电源
				Pin_CFG(PIN_PLCPWR,0+(1<<5)+(1<<17)+(0<<18));//引脚配置
			}
			else
			{//不关电源
				Pin_CFG(PIN_PLCPWR,0+(1<<5)+(1<<17)+(1<<18));//引脚配置
			}
		...
		case 1://复位解除
			if(p16timer[0]==0)
			{
				Pin_OutH(PIN_PLCPWR);//引脚输出高,入口引脚使用名
	...
	switch(UARTCtrl->SubTask)
	{
		...
		case 3:
			...
			{//正确接收
				...
				{//写入数据正确
					Pin_CFG(PIN_ENCtrlModulOut,0+(1<<5)+(1<<17)+(1<<18));//控制模块输出允许
				}
			}
			UARTCtrl->SubTask++;
			break;
		...
		case 8://软件更新
  		...
  		MR(ADDR_DATABUFF,ADDR_E4_ENTRY+(4*64),8);//读开始结束地址
			if(p32[0]==0x00002000)
			{//是I0入口地址
				RxByte=p32[1]-p32[0];
				if((p32[1]>p32[0])&&(RxByte<=0x1000))
				{
	        MR(ADDR_DATABUFF+8,ADDR_E4_ENTRY+(RxByte-8),8);//存储器读
	        if(p32[3]==0xA55A5AA5)
	        {
						//起动文件传输
						RxByte+=255;
		...
		case 9://文件发送帧
			...
			p8txbuff[3]=0;
			MR(((u32)p8txbuff+4),ADDR_E4_ENTRY+(256*UARTCtrl->TxSEQ),256);
		...
		case 10://文件发送返回帧
			...
			{//正确接收
				...
				{//发完
					MC(0,ADDR_E4_ENTRY+(4*64),8);//清缓存区
					UARTCtrl->SubTask=0;
					return;
				}
