...
void NAND_BANK3_FMC_Init(void)
{
...
#else
  hnand.Init.MemoryDataWidth = FMC_NAND_PCC_MEM_BUS_WIDTH_16;
#endif
  ...
  hnand.Info.ZoneSize = 0;
	...
	AttSpaceTiming.HoldSetupTime = 6-1;//6
...
#if IC_MT29F1G==1
	DisplayStringAutoUp(0,9,(u8*)"\x0""MT29F1G");//显示字符串自动向上平移;出口:修改下一显示行并寄存在ADDR_AutoRowCol
#endif
#if IC_MT29F2G==1
	DisplayStringAutoUp(0,9,(u8*)"\x0""MT29F2G");//显示字符串自动向上平移;出口:修改下一显示行并寄存在ADDR_AutoRowCol
#endif
#if IC_MT29F4G==1
	DisplayStringAutoUp(0,9,(u8*)"\x0""MT29F4G");//显示字符串自动向上平移;出口:修改下一显示行并寄存在ADDR_AutoRowCol
#endif
#if IC_MT29F8G==1
	DisplayStringAutoUp(0,9,(u8*)"\x0""MT29F8G");//显示字符串自动向上平移;出口:修改下一显示行并寄存在ADDR_AutoRowCol
#endif
...
//ComSpaceTiming.HoldSetupTime = 5;//1-254;CPU值(1-255)通用存储器保持时间
	i=8;
	while(1)
	{
...
#else//#if NAND_BUS_WIDTH==8//8=8bit,16=16bit
//bus=16bit
//块擦除时间(ms)
	p8=(u8*)0x80000000;
	i=(NAND_BLOCK_COUNT-1)*128*1024;
	i>>=1;
	p8[0x10000]=0x60;//Command
	p8[0x20000]=(i>>10);//Row Add. 1
	p8[0x20000]=(i>>18);//Row Add. 2
#if (NAND_2G|NAND_4G|NAND_8G)
	p8[0x20000]=(i>>26);//Row Add. 3
#endif
	p8[0x10000]=0xD0;//Command
	while(!(FMC_Bank3->SR&(1<<6)));//0：FIFO 非空
	START_STM32F4xx_TIM2();//启动定时器TIM2为时间测试定时器
...
//页读到BUFF时尚(uS)
	...
	p8[4]=ComSpaceTiming.HoldSetupTime;// = 6-1;//1-254;CPU值(1-255)通用存储器保持时间
	...
	if(ComSpaceTiming.WaitSetupTime>2)
	{
		Err=1;
	}
	if(ComSpaceTiming.HoldSetupTime>4)
	{
		Err=1;
	}
...
//		Err=1;
//	}
	if(BlockEraseTimer>3000)//ms;max=3000us
	{
		Err=1;
	}
	if(PageWriteTimer>600)//us;max=600us
	{
		Err=1;
	}
	if(PageReadTimer>70)//us;max=70us
	{
		Err=1;
	}
