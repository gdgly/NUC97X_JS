...
#define ADDR_IRAM_Start     0x00F00000//256K
	#else
	#define ADDR_ms_entry        ADDR_ERAM_linux
	//4byte ADDR_MS_ENTRY
	#define ADDR_os_entry    (ADDR_ms_entry+4)
	//4byte
	#define ADDR_REG_SYS_RSTSTS     (ADDR_os_entry+4)
	//4byte 上电时读取的REG_SYS_RSTSTS值
	#define ADDR_ETHMAC     (ADDR_REG_SYS_RSTSTS+4)//以太网MAC地址
	//6byte MACADDR0
	//6byte MACADDR1
	#endif
	//
#define ADDR_IRAM_Start     (ADDR_ERAM_linux+1024)//(ADDR_ERAM_linux+0x00F00000)
#endif
	

#define ADDR_Comm_Ram_Start     ADDR_IRAM_Start
//公用数据上电初始化清0--------------------------------------------------------------------------------------------
	//4字节留给显示字符转换为点阵LCDDot汇编程序用(仅为固定地址0x10000000要求)
...
typedef __packed struct
{
	...
	u8 NAND;//NAND读写标志
	u8 NAND_ECC;//nand内部ECC,0x55=已允许,其他值=未允许
	u8 osSTATE;//os状态:0=没起动,1=起动,2=暂停
#ifndef LINUX_CROSS_GCC
}MRW_TypeDef;
#else
}__attribute__ ((packed)) MRW_TypeDef;
#endif
...
#else
}__attribute__ ((packed)) FileDownload_TypeDef;
#endif
...
#else
}__attribute__ ((packed)) FilePlay_TypeDef;
#endif
...
#define ADDR_SYSTEM     (ADDR_FilePlay+64)
  ...
  u8 Init_LOG;//上电后LOG已初始化标志:0=没，0x55=正在初始化,0xAA=初始化完成
  ...
  u8 POWER;//电源:0=电池供电,1=电网供电转电池供电重启动,2=电网供电转电池供电,3=电网供电
	u8 outpower;//输出电源B0=A5V0,B1=BACKLIGHT,B2=LED3V3,B3=E5V0,B4=R5V0,B5=USB5V0,B6=2E5V0,B7=中断关电源标志
	u8 Program;//(汇编取地址)当前运行的程序段(用于HardFault中断时重启OS识别):0=空,1=FS,2=USB0,3=USB1,4=ETH,5=...
  u8 Init_OS;//OS需重启原因标志:B0=软件更新,B1=FS,B2=USB0,B3=USB1,B4=ETH,B5=HardFault,B6=,B7=TEST
...
#define ADDR_RouterType     ADDR_WaitFLASHErase+1
	u8 RouterType;//路由器类型:0=保留,1=正泰,2=鼎信,3=东软,4=晓晨,5=瑞斯康,6=力合微,7=讯能,8=钱龙,9=华瑞,10=新开普
...
#define ADDR_RemoteWirelessModuleType     ADDR_RouterType+1
	...
	u8 RemoteWirelessModuleSYSINFO;//远程无线模块当前注册的网络:0=No service,1=,2=2G(GPRS,CDMA),3=3G(WCDMA,TD-SCDMA,CDMA EVD0),4=4G(FDD-LTE,TDD-LTE)
#define ADDR_RamLanguage     ADDR_RemoteWirelessModuleType+2//显示语言(同步FLASH中的ADDR_Language)
...
#define ADDR_CSPN     ADDR_RamLanguage+1//读取SIM卡服务运营商名称
	...
	#if ((ADDR_CSPN+1)&1)
	u8 SPACE1;//为下半字对齐	
#define ADDR_UARText     (ADDR_CSPN+2)
	#else
#define ADDR_UARText     (ADDR_CSPN+1)
	#endif
...
#else
}__attribute__ ((packed)) Comm_Ram_TypeDef;
#endif

#define Comm_Ram     ((Comm_Ram_TypeDef *)(ADDR_SYSTEM))
#define ADDR_Comm_Ram_End     (ADDR_MODE3in1+4)
...
#else
	#define LEN_Terminal_IRAM     (34*1024)//限定总长度字节数
#include "IRAM_Terminal698.h"
#endif
...
#ifdef LINUX
#define ADDR_Meter_SaveRam_Start     (ADDR_IRAM_Start+0x00040000)//256K
#endif
...
#include "IRAM_TerminalSave698.h"
#endif
...
#define ADDR_BKPSRAM     0x00F80000//256k
	#else
#define ADDR_BKPSRAM     (ADDR_IRAM_Start+0x00080000)//256k
	#endif
#endif
