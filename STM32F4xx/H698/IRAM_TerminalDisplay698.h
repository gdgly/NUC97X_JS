...
#else
}__attribute__ ((packed)) MenuCount_TypeDef;
#endif
...
#else
}__attribute__ ((packed)) MenuSave_TypeDef;
#endif
typedef __packed struct
{
...
#else
}__attribute__ ((packed)) MenuStack_TypeDef;
#endif
...
typedef __packed struct
{
	...
	u8 Ln9Code;//屏幕底部(Ln=9)显示内容代号:0=终端抄表状态,1=打开串口,2=检测通信模块,3=检测SIM卡,4=网络注册,5=获取信号,6=开始拨号,7=登录主站,8=登录主站成功
	u8 res2;
#ifndef LINUX_CROSS_GCC
}DOTMIXPMODE_TypeDef;
#else
}__attribute__ ((packed)) DOTMIXPMODE_TypeDef;
#endif
...
#else
}__attribute__ ((packed)) TerminalDisplayLoop_TypeDef;
#endif
...
#else
}__attribute__ ((packed)) StrLnBuff_TypeDef;
#endif
typedef __packed struct
{
...
#else
}__attribute__ ((packed)) StrBuff_TypeDef;
#endif
...
#else
}__attribute__ ((packed)) AutoDisplayRowCol_TypeDef;
#endif
...
#else
}__attribute__ ((packed)) GUI_TTF_DispCode_TypeDef;
#endif
